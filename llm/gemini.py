"""Gemini (Google) LLM analyzer implementation."""
...
class GeminiAnalyzer(LLMAnalyzer):
    """Analyzes email components using Gemini API."""
    ...
    def __init__(self, api_key: str, model: str, sensitivity: int = 5, logger: logging.Logger = None):
        ...
        self.logger = logger or logging.getLogger("MailMind.LLM.Gemini")

    def _query(self, prompt: str) -> AnalysisResult:
        """Send a classification query to Gemini."""
        try:
            # LOGGING: DEBUG-Level für API-Request
            self.logger.debug(f"Sending request to Gemini API (model: {self.model_name})")
            ...
            self.logger.info(
                f"Gemini response: score={score:.2f}, category={category_str}, certain={data.get('is_certain', False)}"
            )
            ...
            return AnalysisResult(
                ...
                raw_data={"provider": "gemini", "model": self.model_name, "response": data}
            )

        except json.JSONDecodeError as e:
            self.logger.warning(f"Failed to parse Gemini response: {e} | Raw: {text[:200]}")
            return AnalysisResult(
                score=0.5,
                category="unknown",
                is_certain=False,
