"""Ollama (self-hosted) LLM analyzer implementation."""
...
class OllamaAnalyzer(LLMAnalyzer):
    """Analyzes email components using Ollama API."""

    def __init__(self, endpoint: str, model: str, sensitivity: int = 5, logger: logging.Logger = None):
        self.endpoint = endpoint.rstrip("/")
        self.model = model
        self.sensitivity = sensitivity
        self.logger = logger or logging.getLogger("MailMind.LLM.Ollama")

    def _query(self, prompt: str) -> AnalysisResult:
        """Send a classification query to Ollama."""
        try:
            # LOGGING: DEBUG-Level für API-Request
            self.logger.debug(f"Sending request to Ollama (endpoint: {self.endpoint}, model: {self.model})")
            ...
            full_prompt = SYSTEM_PROMPT + sensitivity_note + "\n\n" + prompt
            ...
            self.logger.info(
                f"Ollama response: score={score:.2f}, category={category_str}, certain={data.get('is_certain', False)}"
            )
            ...
            return AnalysisResult(
                ...
                raw_data={"provider": "ollama", "model": self.model, "endpoint": self.endpoint, "response": data}
            )

        except json.JSONDecodeError as e:
            self.logger.warning(f"Failed to parse Ollama response: {e} | Raw: {text[:200] if 'text' in locals() else 'N/A'}")
            return AnalysisResult(
                score=0.5,
                category="unknown",
                is_certain=False,
